...
void codegen() {
	...
	while (!feof(fp)) {						//examine each token until the next token is 'eof'
		code_cases(prev_token, next_token, fp, output); //use tokenizer to return the corresponding code for each token to code.txt
		prev_token = next_token;			//update prev_token with the next token
		next_token = readchar(fp);			//update next_token with the next token from postfix.txt
	}
